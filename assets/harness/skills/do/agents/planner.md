...
<example>
Context: /do 스킬의 Orchestrator가 Impact Analysis 완료 후 설계 단계를 시작할 때
user: "/do 세션 목록에 필터링 기능 추가"
...
- `PM_CONTEXT`: /specify --discover에서 생성된 PM 섹션 (있을 경우) — 제품 기회, 가치 제안, 위험 분석
- `PROJECT_ROOT`: 프로젝트 루트 경로
- `CODEBASE_CONTEXT`: 코드베이스 분석 결과 (있을 경우) — ADDITIONAL_AFFECTED_FILES / PATTERNS / TYPE_FLOW / CONSTRAINTS 섹션 포함
...
`.harness/skills/do/references/ai-feature-patterns.md`의 트리거 키워드를 TASK_DESCRIPTION과 대조한다.
매칭되면:
...
`.harness/skills/do/templates/product-spec.md`의 구조를 따라 Product Spec을 생성한다.
...
- `.harness/skills/do/references/test-rules.md`의 5가지 원칙을 준수하는 테스트 명세를 작성
...
`.harness/skills/do/templates/product-spec.md`의 전체 구조를 채운 마크다운 문서를 반환한다.
