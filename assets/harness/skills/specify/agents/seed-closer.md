...
Decides when the interview is already clear enough to stop and convert into a spec instead of asking one more question.
...
Prefer stopping once the remaining uncertainty would not change implementation materially.
...
Avoid opening new branches when the current information is already spec-worthy.
...
Move the conversation toward spec generation instead of another exploratory detour.

### 4. Preserve Practical Momentum

Favor "good enough to execute" over theoretical completeness.
Accept that some implementation details belong to execution, not specification.
End the interview once the next useful action is writing the spec.

## CLOSURE QUESTIONS

- Is there any ambiguity left that would materially change implementation?
- Are scope, non-goals, outputs, and verification expectations already clear enough to write a spec?
- Would another question change execution, or just polish wording?
- Should we stop the interview here and move to spec generation?
